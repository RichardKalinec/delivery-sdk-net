...
```csharp
// Initializes an instance of the DeliveryClient client by building it with DeliveryClientBuilder
DeliveryClient client = DeliveryClientBuilder.WithProjectId("YOUR_PROJECT_ID").Build();
```

You can also provide the project ID and other parameters by passing the [`DeliveryOptions`](https://github.com/Kentico/delivery-sdk-net/blob/master/KenticoCloud.Delivery/Configuration/DeliveryOptions%20.cs) object to the DeliveryClientBuilder. The `DeliveryOptions` object can be built with DeliveryOptionsBuilder. It is used to set the following parameters:

* `WithProjectId` – sets the project identifier. (mandatory step)
...
* `UseProductionApi` - makes DeliveryClient use the default Delivery API.
...
* `WithMaxRetryAttempts` - sets the maximum retry attempts.
* `WithCustomEndpoint` - allows usage of a custom endpoint.

```csharp
// Creates the deliveryOptions object first
...
// Initializes an instance of the DeliveryClient client with the specified deliveryOptions
DeliveryClient client = DeliveryClientBuilder.BuildWithDeliveryOptions(deliveryOptions);
```
